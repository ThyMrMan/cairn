...
**Resuming continues the capture it was, not the site.** The resume job used to say only which capture to continue, so everything else came from the site: the discovered URL set, the site's depth, the site's engine. A paused feed capture — a handful of posts at depth 0 — came back as a crawl of the whole site, into the feed capture's directory, and the feed items it was for were never marked. The capture now keeps what it was asked for (`captures.request`, [03](03-data-model-and-storage.md#captures--urls)), the resume endpoint copies it into the job, and the job runner reads it in place of the job's own spec. The copy in the job is what the documented fallback needs: if the capture is deleted while the resume waits, the job becomes a fresh capture — of the same posts.

It is kept on the capture because the job that knew does not last. Pause leaves its job `ok`, clearing the job list deletes finished jobs, and `captures.job_id` is `ON DELETE SET NULL`. A capture paused before the column existed is read off its job while the job is there; with neither, a full crawl can still continue as one, and anything else is refused (`409 resume_unknown`) rather than continued as the site.

**A pause is not a failed capture.** A paused feed capture used to settle its items as a failure: back to pending, and the feed's capture backoff moved on. Once the backoff ran out, the next tick captured the same posts again beside the paused capture. Its items now stay pending and are held — not dispatched — for as long as the capture is paused ([08](08-feeds-and-scheduling.md#incremental-captures)); new posts are captured as usual meanwhile.

**The engine asked is the one the job runs.** `can_pause` and the pause endpoint asked the *site's* engine, and a companion pass brings its own: the Blogger pagination pass runs wget on a site that may be set to browsertrix. Pause was offered on it, and the click landed as an ordinary stop — the quiet downgrade the endpoint exists to refuse. They now ask the capture's engine once there is a capture, and the pass's before that.
...
>
> **`--exclude` filters the page queue, not the network.** Its own help says "regex of **page URLs**", and that is the whole of it: a beacon fired by the page's JavaScript is never queued as a page, so no exclude rule can touch it. `/b/stats` therefore stayed at 26% of every fetch across three real captures with an exclude pattern that matched it exactly. `--blockRules` is the network-level one — a plain string there is read as a URL regex with type `block` — and cairn now passes reject patterns to both. Measured on a fixture whose page fetches a beacon from script: `--exclude` alone, 4 records with the beacon archived; with `--blockRules`, 3 and gone. This also makes the two engines agree, since wget's `--reject-regex` has always applied to everything it fetches.
>
