...
**Mounting it is not the same as being able to open it,** and the preflight used to conflate the two. On Unraid the socket is group-owned by `docker` on the host, that group does not exist in this image, and the template runs as 99:100 — so the mount succeeds, the check passed, and the capture died several frames deep in httpx with `[Errno 13] Permission denied`. The check now tests access as well as existence and reports the socket's uid, gid and mode alongside the process's own, because `--group-add <gid>` is the fix and the gid is the part you cannot guess. **This is a real grant, not a formality:** adding that group gives this container the root-equivalent access described above. `jobs.allow_docker` exists so an operator who mounted the socket for something else can still forbid engines from using it; it defaults to on, because mounting the socket is already the deliberate act and a second switch defaulting to off would mean somebody does the work and is told the feature is unavailable with no hint why.
