...
4. **browsertrix cannot take a cookie jar, and the bridge is a tarball.** It has no cookie option; `--profile` wants a browser profile tarball, and one built with our Chromium is accepted and ignored because it runs **Brave** while we ship **Chrome for Testing**. Verified against a gated fixture: it archived the interstitial. So "profiles" is struck from the bullet list above, and the picker warns before the capture. **Corrected after M7:** the conclusion "no bridge exists" was one step short — the crawler's own `create-login-profile` drives the same browser, so its tarball *is* read. The engine now declares `auth: [user_agent, browser_profile]` and Cairn stores the tarball rather than building one. It is also headful under Xvfb, which is why it gets through a Google sign-in that docs/06 records as out of reach for the in-app browser. Proven end to end against 1.14.1 with a real 41 MB profile: mounted at `/cairn/auth`, passed as `--profile`, logged "With Browser Profile", archived a 200.
...
3. **A scanner that can fail the build is a scanner that gets switched off.** The image pins `setuptools<81` because pywb imports `pkg_resources`, and the first Trivy run found exactly one HIGH — Werkzeug 2.2.3, which pywb requires *exactly*, in a release that is the newest there is. A blocking gate would have been red from the day it was added, on a finding no bump of ours could clear from *within* the constraint. They report to the run summary instead, and run weekly, because the advisories arrive while the code sits still.

   That finding was then cleared by leaving the constraint: the image installs `werkzeug>=3.0.3` over pywb's exact pin, and the scan now reports zero HIGH or CRITICAL with fixes available. Measured before it was done — pywb replays byte-for-byte identically on 3.1.8 across the bare content, the framed wrapper, the untimestamped redirect and the CDX API — and, more importantly, guarded after: `test_replay_e2e.py` and `test_thumbnail.py` drive a real pywb on every container run, so a future pywb that genuinely needs 2.2.3 fails a test rather than a replay tab. Overriding an upstream pin is only defensible when something else is watching it.
...
**Found while measuring it.** One capture on the same instance fetched 4,011 distinct URLs **381,126 times**, and nothing reported that either. wget remembers what it has fetched by the files it left on disk, so anything writing no file is asked for again every time it is linked — the same fact that makes `--delete-after` unusable here, in a form nobody had looked for. The repeat count is now recorded on every capture and narrated by `crawlhealth`, which the job page already polls mid-crawl — asking it rather than counting again is what stops the capture report and the job page disagreeing about one crawl. Its 3.0x bar was already measured against the case a lower one would spoil: a site reachable under two names sits at exactly 2.0x and is harmless.
